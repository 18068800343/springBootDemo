...
public final class DebugOverlay {
    ...
    private static volatile String direction="—",detail="等待小地图…";private static volatile float confidence;private static volatile int playerX=-1,playerY=-1,targetX=-1,targetY=-1;private static volatile boolean road;
    private DebugOverlay(){}
    public static boolean show(AutoBrushAccessibilityService s){if(s==null||!Settings.canDrawOverlays(s))return false;service=s;if(panel!=null)return true;wm=(WindowManager)s.getSystemService(Context.WINDOW_SERVICE);panel=new Panel(s);lp=new WindowManager.LayoutParams(dp(330),dp(455),android.os.Build.VERSION.SDK_INT>=26?WindowManager.LayoutParams.TYPE_APPLICATION_OVERLAY:WindowManager.LayoutParams.TYPE_PHONE,WindowManager.LayoutParams.FLAG_NOT_FOCUSABLE|WindowManager.LayoutParams.FLAG_LAYOUT_NO_LIMITS,android.graphics.PixelFormat.TRANSLUCENT);lp.gravity=Gravity.TOP|Gravity.RIGHT;lp.x=dp(8);lp.y=dp(70);try{wm.addView(panel,lp);return true;}catch(Throwable t){panel=null;return false;}}
    ...
    public static void update(Bitmap m,MapNavigator.Result r,String status){if(m!=null&&!m.isRecycled()){Bitmap c=m.copy(Bitmap.Config.ARGB_8888,false);Bitmap old=map;map=c;if(old!=null&&!old.isRecycled())old.recycle();}if(r!=null){playerX=r.playerX;playerY=r.playerY;targetX=r.targetX;targetY=r.targetY;confidence=r.confidence;road=r.roadDirection;direction=dir(r.dx,r.dy);}detail=status==null?"":status;if(panel!=null)panel.postInvalidate();}
    ...
    private static final class Panel extends View{final Paint p=new Paint(Paint.ANTI_ALIAS_FLAG);float downX,downY;int startX,startY;Panel(Context c){super(c);setLayerType(View.LAYER_TYPE_SOFTWARE,null);}
        protected void onDraw(Canvas c){super.onDraw(c);float d=getResources().getDisplayMetrics().density;p.setStyle(Paint.Style.FILL);p.setColor(0xEE101010);c.drawRoundRect(new RectF(0,0,getWidth(),getHeight()),18*d,18*d,p);p.setTypeface(Typeface.DEFAULT_BOLD);p.setTextSize(19*d);p.setColor(Color.WHITE);c.drawText("自动刷图31 · 寻路调试",14*d,27*d,p);p.setTypeface(Typeface.DEFAULT);p.setTextSize(12*d);p.setColor(0xFFAAAAAA);c.drawText("实时原始小地图 + 规划结果",14*d,46*d,p);
            ...
            p.setStyle(Paint.Style.FILL);p.setTypeface(Typeface.DEFAULT_BOLD);p.setTextSize(18*d);p.setColor(road?0xFF62E28B:0xFFFFC857);c.drawText("方向："+direction,14*d,294*d,p);p.setTypeface(Typeface.DEFAULT);p.setTextSize(12*d);p.setColor(0xFFE0E0E0);c.drawText(String.format(java.util.Locale.US,"人物 (%d,%d)  置信度 %.0f%%",playerX,playerY,confidence*100),14*d,316*d,p);c.drawText("当前："+(detail.length()>44?detail.substring(0,44):detail),14*d,338*d,p);
