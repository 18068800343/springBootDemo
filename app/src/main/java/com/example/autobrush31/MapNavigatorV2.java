...
/** Planner v2: gray-white road is walkable; deep black is fog; dark-gray is non-road. */
public final class MapNavigatorV2 {
    ...
    public static Result analyze(Bitmap s,int prevDx,int prevDy){
        ...
        o.debug=dbg.length()==0?"没有连续灰白道路":dbg.toString();return o;
    }
    ...
    private static int[] findPlayer(Bitmap s,int w,int h){
        ...
        for(int y=1;y<h-1;y++)for(int x=1;x<w-1;x++)if(m[y*w+x]&&!seen[y*w+x]){
            ...
            int area=(maxx-minx+1)*(maxy-miny+1);if(n<12||area>5000)continue;
