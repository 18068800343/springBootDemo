...
public class AutoBrushAccessibilityService extends AccessibilityService {
 ...
 private final Runnable loop=new Runnable(){@Override public void run(){if(!running)return;long now=System.currentTimeMillis();if(!GAME.equals(foregroundPackage)){status="等待迷雾大陆前台…";postNext(500);return;}if(phase==0){status="选择源初秘境";tap(SECRET_X,SECRET_Y);phase=1;phaseAt=now;}else if(phase==1&&now-phaseAt>1800){status="选择地狱31";tap(HELL_X,HELL_Y);phase=2;phaseAt=now;}else if(phase==2&&now-phaseAt>1800){status="进入地狱31";tap(ENTER_X,ENTER_Y);phase=3;phaseAt=now;}else if(phase==3&&now-phaseAt>2500&&now-lastShotAt>850){shot();lastShotAt=now;}postNext(phase==3?350:250);}};
 private void postNext(long delay){handler.postDelayed(loop,delay);}
